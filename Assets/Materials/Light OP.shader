...
////// Emissive:
                ...
                float node_5267 = pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_Gloss)));
                float3 finalColor = emissive + ((node_2337*floor((lerp(_Shadow_Color.rgb,_Light_Color.rgb,saturate((saturate((_T_noise_var.r+node_7782))*node_7782)))+(node_5267*_SpecColor.rgb)) * node_3168) / (node_3168 - 1))*_LightColor0.rgb*attenuation);
                fixed4 finalRGBA = fixed4(finalColor,(_Opacity+(node_7782*node_5267)));
...
////// Lighting:
                ...
                float node_7782 = max(0,dot(lightDirection,normalDirection)); // Lambert
                float node_5267 = pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_Gloss)));
                float3 finalColor = ((node_2337*floor((lerp(_Shadow_Color.rgb,_Light_Color.rgb,saturate((saturate((_T_noise_var.r+node_7782))*node_7782)))+(node_5267*_SpecColor.rgb)) * node_3168) / (node_3168 - 1))*_LightColor0.rgb*attenuation);
                fixed4 finalRGBA = fixed4(finalColor * (_Opacity+(node_7782*node_5267)),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
